...
priority_green_stages = [
    ...
    [float(152.0), float(154.0), float(160.0), float(42.0), float(8.1), "train_and_bus_green_stage"]
]
...
def handle_connection():
    ...
    while True:
        ...
        if deserialized_JSON is not None:
            ...
            if tick / 2 == motorized_vehicle_time + orange_wait_time + clear_traffic_time:
                tick = 0
                current_green_stage = get_green_stage(deserialized_JSON)
                set_lights_green(current_green_stage)
                server.send(encoder.serialize(traffic_lights_array.traffic_lights))
...
def check_vehicle_priorities():
    # train prio
    if vehicle_priorities.check(float(152.0)) or vehicle_priorities.check(float(154.0)) or vehicle_priorities.check(float(160.0)):
